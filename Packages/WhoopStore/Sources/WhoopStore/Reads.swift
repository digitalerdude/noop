...
extension WhoopStore {
    ...
    public func hrFingerprint(deviceId: String, from: Int, to: Int) async throws -> (count: Int, maxTs: Int) {
        try syncRead { db in
            ...
            // exactly one row, so fetchOne is non-nil and the columns read straight into Int (the proven
            // `$0["col"]` style used by hrBuckets/rrIntervals above). The guard is belt-and-suspenders.
